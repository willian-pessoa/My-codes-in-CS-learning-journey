...
class PhraseTrigger(Trigger):
    def __init__(self, phrase):
        """  
        Receive a phrase with just one space betwen wods and no punctuation
        """
    ...
    def is_phrase_in(self, text):
        """ 
        Enter a text and check if the phase is in text
        """
        ...
        for i in range(len(test) - 1):
            if test[i + 1] - test[i] != 1: #check if the words in sequence,if not return false
                return False
...
class TimeTrigger(Trigger):
    def __init__(self, pubtime):
        formato = '%d %b %Y %H:%M:%S'
        pubtime = datetime.strptime(pubtime, formato)
...
def filter_stories(stories, triggerlist):
    ...
    print("trig_stories:",trig_stories)
    return trig_stories
...
def main_thread(master):
    ...
    try:
        t1 = TitleTrigger("Bolsonaro")
        t2 = DescriptionTrigger("perdendo")
        t3 = DescriptionTrigger("dinheiro")
        t4 = AndTrigger(t2, t3)
        triggerlist = [t1, t4, t2]
        ...
        while True:
            ...
            stories = process("https://news.google.com/rss?hl=pt-BR&gl=BR&ceid=BR:pt-419")
